...
export interface IAuthService {
     ...
     * iframe — the {@link AuthGate} renders the standalone login screen in
     * that case.
     */
    initEmbeddedAuth(): Promise<OpaqueSession | null>;

    /**
     * Interactive standalone sign-in — opens the Fabric broker popup for a
     * user gesture (button click). Used by {@link AuthGate} when the app is
     * opened outside the Fabric portal iframe.
     */
    signInInteractive(): Promise<OpaqueSession>;
}
...
class RayfinAuthService implements IAuthService {
    ...
    async signInInteractive(): Promise<OpaqueSession> {
