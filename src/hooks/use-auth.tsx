...
export function AuthProvider({ children, rayfinAuthService }: AuthProviderProps) {
    ...
    // Interactive sign-in failures (popup blocked, user cancelled, timeout) are
    // recoverable — shown inline on <AuthGate>, not thrown to the ErrorBoundary.
    ...
    const signIn = async () => {
        ...
        setSignInError(null);
        try {
            const result = await rayfinAuthService.signInInteractive();
            setSession(result);
        } catch (err) {
    ...
    const value = useMemo<AuthContextValue>(
        ...
        [session, isLoading, signInError, isSigningIn],
    );
