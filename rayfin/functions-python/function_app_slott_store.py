...
def _fetch_pinnacle_matchups(sport_id):
    path = f"/sports/{sport_id}/matchups?withSpecials=false&brandId=0"
    url = PINNACLE_API + path
    req = urllib.request.Request(url, headers=PINNACLE_HEADERS)
    try:
        with urllib.request.urlopen(req, timeout=45) as resp:
            raw = resp.read().decode("utf-8")
            if raw.lstrip().startswith("<") or "cf_chl" in raw or "Just a moment" in raw:
                raise fn.UserThrownError("Pinnacle : réponse bloquée (Cloudflare)")
            return json.loads(raw)
    except urllib.error.HTTPError as exc:
        raise fn.UserThrownError(f"Pinnacle HTTP {exc.code}") from exc
    except urllib.error.URLError as exc:
        raise fn.UserThrownError(f"Pinnacle réseau : {exc.reason}") from exc
...
@udf.function()
def fetchPinnacleMatchups(sportId: str) -> str:
    logging.info("fetchPinnacleMatchups sportId=%s", sportId)
    return json.dumps(_fetch_pinnacle_matchups(str(sportId)))
